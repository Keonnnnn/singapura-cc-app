...
function CustomerEvents() {
  ...
  useEffect(() => {
    const fetchEventsAndRegistrations = async () => {
      try {
        ...
        if (userId) {
          const registrationResponse = await http.get(
  ...
  const formik = useFormik({
    ...
    onSubmit: (data, { resetForm }) => {
      ...
      http
        .post("/events/eventrequests", data)
        .then((res) => {
    ...
    <Container sx={{ flexGrow: 1, py: 4 }}>
              ...
              <Box sx={{ flexGrow: 1, position: "relative" }}>
                <CardContent sx={{ p: 0 }}>
                  ...
                  <Typography variant="body2" color="text.secondary">
                    ...
                    {dayjs(event.endTime, "HH:mm").format("hh:mm A")}
                  </Typography>
                  <Chip
                    label={event.type}
                    sx={{
                  ...
                  {!token && (
                    ...
                    >
                      Only members can register for events. Walk-ins are
      ...
      <Box sx={{ mt: 6, borderRadius: 1, backgroundColor: "#f9f9f9", p: 4 }}>
        ...
        <Typography variant="body1" sx={{ mb: 4 }}>
          Do you have an exciting sporting programme coming up that you’d like
          to share with us? Simply submit your event below, and we’ll carefully
          review it. Please keep in mind that all submissions are subject to
          approval by our Editorial Team.
        </Typography>
        <Box
          component="form"
