...
const FeedbackList = () => {
    ...
    const calculateTotalLikes = (feedbacks) => {
        ...
        const total = userFeedbacks.reduce((acc, feedback) => acc + feedback.likes, 0);
        setTotalLikes(total);
    };
        ...
        <Container maxWidth="md" sx={{ mt: 4 }}>
            <Paper elevation={3} sx={{ p: 3, borderRadius: '12px', position: 'relative' }}>
                ...
                {feedbackList.length === 0 ? (
                    <Typography align="center" sx={{ color: '#757575' }}>No feedback available</Typography>
                ) : (
                    <List>
                        {feedbackList.map(feedback => (
                            <React.Fragment key={feedback.id}>
                                ...
                                >
                                    <Box sx={{ display: 'flex', alignItems: 'center', flexGrow: 1 }}>
                                        <ListItemAvatar>
                                            <Avatar sx={{ bgcolor: '#b71c1c', color: '#fff' }}>
                                                <Box component="a" href={`/feedback/${feedback.id}`} sx={{ textDecoration: 'none', color: '#fff' }}>
                                                    {feedback.userId}
                                                </Box>
                                            </Avatar>
                                        </ListItemAvatar>
                                    ...
                                    {feedback.imageFile && (
                                        <Box sx={{ width: 150, height: 100, overflow: 'hidden', borderRadius: '8px', ml: 2 }}>
                                            <img alt="feedback" src={`${import.meta.env.VITE_FILE_BASE_URL}${feedback.imageFile}`} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                                        </Box>
                                    )}
                                    <Box sx={{ display: 'flex', alignItems: 'center' }}>
                                        <Tooltip title="Like Feedback">
                                            <IconButton 
                                                color={likedFeedbacks.has(feedback.id) ? "error" : "default"} 
