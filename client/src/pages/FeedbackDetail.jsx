...
const FeedbackDetail = () => {
    ...
    useEffect(() => {
        const fetchFeedback = async () => {
            try {
                const res = await axios.get(`http://localhost:3001/feedback/${id}`);
    ...
    const handleUpdate = async () => {
        try {
            await axios.put(`http://localhost:3001/feedback/${id}`, {
                userId,
                eventId,
                content,
                response
            ...
            navigate('/feedbacks', { replace: true });  // Navigate back to the feedback list
        } catch (error) {
