...
import { Box, Typography, Grid, Card, CardContent, Paper, Avatar, Chip, Container, Button } from '@mui/material';
...
function UserRegistrationHistory() {
    ...
    useEffect(() => {
        const fetchRegistrations = async () => {
            try {
                const response = await http.get(`/events/user/${userId}/registrations`, {
                    ...
                    {registrations.map((registration, index) => (
                        <Grid item xs={12} key={index}>
                            <Card sx={{ display: 'flex', alignItems: 'center', p: 2, borderRadius: 2 }}>
                                <Box component="img" src={`${import.meta.env.VITE_FILE_BASE_URL}${registration.Event.imageFile}`} alt={registration.Event.name} sx={{ width: 240, height: 200, mr: 2, borderRadius: 1 }} />
                                <Box sx={{ flexGrow: 1, position: 'relative' }}>
                                    <CardContent sx={{ p: 0 }}>
                                        ...
                                        <Typography variant="body2" color="text.secondary">
                                            {dayjs(registration.Event.startTime, 'HH:mm').format('hh:mm A')} - {dayjs(registration.Event.endTime, 'HH:mm').format('hh:mm A')}
                                        </Typography>
                                        <Chip label={registration.Event.type} sx={{ mt: 1, mb: 1, bgcolor: 'primary.light', color: 'white' }} />
